  ...
  >
    ...
    >
      <div
        ...
        :class="{
          'icons-full': card.style && card.style.isFull && card.style.bgUrl,
        }"
      >
            ...
            <li v-for="label in myLabels" :key="label.id">
              ...
              >
                <transition name="fade">
                  <span class="label-title" v-if="label.title && isLabelOpen">{{
                    label.title
                  }}</span>
                </transition>
              </span>
        ...
        <div class="icon-list" :class="{ colum: card.members.length > 1 }">
          ...
          <div
            class="members"
            v-if="card.members && card.members.length > 0 && !card.style.isFull"
          >
            <div
...
export default {
  ...
  methods: {
    ...
    toggleLabel() {
      let labelToggle = JSON.parse(
        JSON.stringify(this.$store.getters.isLabelOpen)
      );
      labelToggle = !labelToggle;
      this.$store.commit({
        type: "updateIsLabelOpen",
        isLabelOpen: labelToggle,
      });
