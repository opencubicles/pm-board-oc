<template>
  <section class="main-boards">
    <div
      class="flex viewvd"
      v-if="userBoards && userBoards.boardsStar && userBoards.boardsStar.length"
    >
    ...
    >
      <div v-for="board in userBoards.boardsStar" :key="board._id">
        ...
        >
          ...
          <h3 class="selected" @click.stop="changeFavorit(board._id, true)">
            <span class="selected"></span>
          </h3>
        </div>
      </div>
    </div>
    <div v-if="userBoards && userBoards.boards && userBoards.boards.length">
      <i class="bi bi-clock icon"></i>
      <strong style="margin-top: 0.5rem"> Recently viewed </strong>
    </div>
    <div>
      <div
        class="boards-container flex"
        v-if="userBoards && userBoards.boards && userBoards.boards.length"
      >
        <div v-for="board in userBoards.boards" :key="board._id">
          <div
            ...
            @click="openBoard(board._id)"
          >
            <strong class="card-title" style="color: white; padding: 0.5rem">{{
              board.title
            }}</strong>
            <h3
              class="selected-fevorit hover"
              @click.stop="changeFavorit(board._id, false)"
            >
      ...
      <div class="personal-boards flex viewvd" style="gap: 0.5rem">
      ...
      <div class="boards-container flex">
        <div
          class="card-board flex-center pointer create-new"
          @click="CreateBoard"
        >
          <strong class="card-title">Create new board</strong>
        </div>
      </div>
    </div>
    <main-template />
